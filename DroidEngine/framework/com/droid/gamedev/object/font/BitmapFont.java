...
public class BitmapFont implements GameFont {
	 ...
	 */
	public BitmapFont(GameImage[] imagefont, String letterSequence) {
		...
		for (int i = 0; i < totalLetter; i++) {
			this.charIndex[letterSequence.charAt(i)] = i;
		}
	}
	 ...
	 * @param imagefont the images font, all images must have same size
	 */
	public BitmapFont(GameImage[] imagefont) {
	...
	public int drawString(GameGraphics g, String s, int x, int y) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (index != -1) {
				try {
					g.drawImage(this.imagefont[index], x, y);
	 ...
	 */
	public void setImageFont(GameImage[] imagefont, String letterSequence) {
		this.imagefont = imagefont;
