...
public class GameLoader implements Runnable{
	 ...
	 */
	public void setup(Game game, int width, int height, boolean fullscreen,	boolean bufferstrategy) {

		// first thing is definitely use platform = Window
		GameLoader.platform = GamePlatform.AWT;
		
		//associate the game
		this.game = game;
		
		// create the game exit listener
		this.game.setExitListner(new GameExitListenerWin());

		// time to create the graphics engine
		try {
			if (fullscreen) {
				// full screen mode
				GFXWinFullScreen mode = new GFXWinFullScreen(width, height,	bufferstrategy);
				this.gfx = mode;
			} 
			else {
				// windowed mode
	...
	private class GameExitListenerWin implements GameExitListener {
	...
	private void initEngineWin(){
		...
		if (this.game.bsLoader == null) {
			this.game.bsLoader = new BaseLoaderWin(this.game.bsIO, GameColor.MAGENTA);
		}
