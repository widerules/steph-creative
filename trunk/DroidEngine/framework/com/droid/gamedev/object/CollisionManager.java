...
public abstract class CollisionManager {
	 ...
	 * <code>y2</code>.
	 */
	public static boolean isPixelCollide(double x1, double y1, GameImage imageData, double x2, double y2, GameImage imageData2) {
		// initialization
		double width1 = x1 + imageData.getWidth() - 1, height1 = y1
		        + imageData.getHeight() - 1, width2 = x2 + imageData2.getWidth() - 1, height2 = y2
		        + imageData2.getHeight() - 1;
		...
		for (int y = 1; y < toty - 1; y++) {
			...
			for (int x = 1; x < totx - 1; x++) {
				...
				try {
					if (((imageData.getRGB(nx, ny) & 0xFF000000) != 0x00)
					        && ((imageData2.getRGB(nx1, ny1) & 0xFF000000) != 0x00)) {
						// collide!!
						return true;
					}
				}
